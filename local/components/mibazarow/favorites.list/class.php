...
class mibazarowFavoritesListComponent extends CBitrixComponent
{
	/**
	...
	protected array $arElementIds = array();
	...
	protected function setTitle(): void
	{
		...
		if ($this->arParams["SET_TITLE"])
			$APPLICATION->SetTitle(Loc::getMessage("mibazarow_FAVORITES_DEFAULT_TITLE"));
	}
	 ...
	 * @return void
	 */
	protected function performActionDeleteFavor(): void
	{
		if (strlen($this->requestData["ID"]) && $this->requestData["DELETE_FAVOR"] == "Y")
		{
			if($id = $this->getRealId($this->requestData["ID"]))
				...
				throw new Main\SystemException(Loc::getMessage('mibazarow_FAVORITES_CANNOT_DELETE_FAVORITES'));
		}
	}
	 ...
	 * @return int Order id
	 */
	private function getRealId($id): int
	{
		$res = \mibazarow\Favorites\FavoritesTable::getById($id);
  		$return = 0;
	 ...
	 * @return void
	 */
	protected function deleteFavor(int $id): void
	{
		global $DB;
		if ($id)
		{
			$DB->StartTransaction();
			$result = \mibazarow\Favorites\FavoritesTable::delete($id);
			if(!$result->isSuccess())
			{
			  $DB->Rollback();
			  throw new Main\SystemException(Loc::getMessage('mibazarow_FAVORITES_CANNOT_DELETE_FAVORITES'));
	 ...
	 * @return void
	 */
	protected function obtainDataFavoritess(): void
	{
		...
		$this->dbQueryResult['FAVORITES'] = \mibazarow\Favorites\FavoritesTable::getList(
			array(
	 ...
	 * @return void
	 */
	protected function formatDate(array &$arr, array $conversion): void
	{
